...
if (kontrol == "TR_tr") {
    ...
    if (!settingsList.includes(settingsSelect)) return message.channel.send(new Discord.MessageEmbed().setDescription(`**Error:** This is not a valid setting.`).setColor(message.guild.me.displayColor));
    else if (!db.fetch(`levelSystem_${message.guild.id}.status`) && settingsSelect !== "durum") return message.channel.send(new Discord.MessageEmbed().setDescription(`**Error:** The level system is turned off.`).setColor(message.guild.me.displayColor));
    else {
        ...
        if (settingsSelect === "add") {
            const [, , level] = args;

            if (!levelRoles || !level || !parseInt(level)) return message.channel.send(new Discord.MessageEmbed().setDescription(`**Error:** You need to enter a setting.`).setColor(message.guild.me.displayColor));
            if (!message.guild.roles.cache.has(levelRoles.id)) return message.channel.send(new Discord.MessageEmbed().setDescription(`**Error:** The role you specified does not exist on the server.`).setColor(message.guild.me.displayColor));
            if (levelSystem.ranks && levelSystem.ranks.find(x => x.roleId === levelRoles.id && x.level === level)) return message.channel.send(new Discord.MessageEmbed().setDescription(`**Error:** This level role has already been created.`).setColor(message.guild.me.displayColor));


            message.channel.send(new Discord.MessageEmbed().setDescription(`Successfully added.`).setColor(message.guild.me.displayColor));
            await db.push(`levelSystem_${message.guild.id}.ranks`, { roleId: levelRoles.id, level });

        } else if (settingsSelect === "remove") {

            const [, , level] = args;

            if (!levelRoles || !level || !parseInt(level)) return message.channel.send(new Discord.MessageEmbed().setDescription(`**Error:** You need to specify a setting.`).setColor(message.guild.me.displayColor));
            if (!levelSystem.ranks && !levelSystem.ranks.find(x => x.roleId === levelRoles.id && x.level === level)) return message.channel.send(new Discord.MessageEmbed().setDescription(`**Error:** There is no such level role.`).setColor(message.guild.me.displayColor));

            const filtered = levelSystem.ranks.filter(x => x.roleId !== levelRoles.id && x.level !== level);

            message.channel.send(new Discord.MessageEmbed().setDescription(`Transaction Successful.`).setColor(message.guild.me.displayColor));
            db.set(`levelSystem_${message.guild.id}.ranks`, filtered);

        } else if (settingsSelect === "list") {

            const ranks = levelSystem.ranks.sort((x, y) => y.level - x.level).map(rank => `${message.guild.roles.cache.find(x => x.id === rank.roleId) ? message.guild.roles.cache.find(x => x.id === rank.roleId).name : "Deleted role"} , ${rank.level}`);
            const rankListing = new MessageEmbed()
                .setTitle(message.guild.name + " Level Roles List")
                .setDescription(ranks)
                .setColor(message.guild.me.displayColor)
                .setThumbnail(message.guild.iconURL())
            message.channel.send(rankListing);
        } else if (settingsSelect === "status") {

            const [, statusSelect] = args;

            if (!["on", "off", "close"].includes(statusSelect)) return message.channel.send(new Discord.MessageEmbed().setDescription(`**Error:** You need to specify whether you want to turn the level system on or off.`).setColor(message.guild.me.displayColor));
            else {
                if (statusSelect === "on") db.set(`levelSystem_${message.guild.id}.status`, true);
                else if (statusSelect === "off" || statusSelect === "close") db.set(`levelSystem_${message.guild.id}.status`, false);

                message.channel.send(new Discord.MessageEmbed().setDescription(`Level status set successfully.`).setColor(message.guild.me.displayColor));
            ...
            if (!["channel", "message"].includes(logCategorySelect)) return message.channel.send(new Discord.MessageEmbed().setDescription(`**Error:** There is no such log category.`).setColor(message.guild.me.displayColor));
            else {
                if (logCategorySelect === "channel") {

                    const levelLogChannel = message.mentions.channels.first();
                    if (!levelLogChannel || !client.channels.cache.get(levelLogChannel.id)) return message.channel.send(new Discord.MessageEmbed().setDescription(`**Error:** No such channel exists.`).setColor(message.guild.me.displayColor));
                    else {

                        await db.set(`levelSystem_${message.guild.id}.logChannel`, levelLogChannel.id);
                        message.channel.send(new Discord.MessageEmbed().setDescription(`Channel set up successfully.`).setColor(message.guild.me.displayColor));

                    }

                } else if (logCategorySelect === "message") {

                    const [, , logMessageSelect] = args;
                    const value = args.splice(3).join(" ");

                    if (!["role", "rank"].includes(logMessageSelect)) return message.channel.send(new Discord.MessageEmbed().setDescription(`**Error:** There is no such log category.`).setColor(message.guild.me.displayColor));
                    else {
                        if (logMessageSelect === "rank") {

                            if (!value) return message.channel.send(new Discord.MessageEmbed().setDescription(`**Error:** You must write the message to be adjusted.`).setColor(message.guild.me.displayColor));
                            else {

                                await db.set(`levelSystem_${message.guild.id}.logUpMessage`, value);
                                message.channel.send(new Discord.MessageEmbed().setDescription(`Successfully level up message set, \n\nLevel message : **${value}**`).setColor(message.guild.me.displayColor));

                            }

                        } else if (logMessageSelect === "role") {
                            if (!value) return message.channel.send(new Discord.MessageEmbed().setDescription(`**Error:** You need to write the message to be adjusted.`).setColor(message.guild.me.displayColor));
                            else {

                                await db.set(`levelSystem_${message.guild.id}.logRewardMessage`, value);
                                message.channel.send(new Discord.MessageEmbed().setDescription(`Successfully set level gift message, \n\nAdjust level message : **${value}**`).setColor(message.guild.me.displayColor));
        ...
        } else if (settingsSelect === "obstacle") {
            const [, categorySelect] = args;

            if (!["channel", "role"].includes(categorySelect)) return message.channel.send(new Discord.MessageEmbed().setDescription(`**Error:** There is no such obstacle category.`).setColor(message.guild.me.displayColor));
            else {

                if (categorySelect === "channel") {
                    ...
                    if (args[2] === "remove") {
                        ...
                        const { blockChannels } = db.fetch(`levelSystem_${message.guild.id}`);
                        if (!blockChannels || !blockChannels.find(x => x === value.id)) return message.channel.send(new Discord.MessageEmbed().setDescription(`**Error:** No such channel is blocked.`).setColor(message.guild.me.displayColor));

                        blockChannels.pop(value.id)
                        db.set(`levelSystem_${message.guild.id}.blockChannels`, blockChannels);
                        message.channel.send(new Discord.MessageEmbed().setDescription(`Successfully deleted.`).setColor(message.guild.me.displayColor));

                    } else {

                        const { blockChannels } = db.fetch(`levelSystem_${message.guild.id}`);

                        if (blockChannels && db.fetch(`levelSystem_${message.guild.id}.blockChannels`).find(x => x === value.id)) return message.channel.send(new Discord.MessageEmbed().setDescription(` **Error:** This obstacle is already set.`).setColor(message.guild.me.displayColor));
                        if (!message.guild.channels.cache.has(value.id)) return message.channel.send(new Discord.MessageEmbed().setDescription(`**Error:** No such channel exists.`).setColor(message.guild.me.displayColor));

                        await db.push(`levelSystem_${message.guild.id}.blockChannels`, value.id);
                        message.channel.send(new Discord.MessageEmbed().setDescription(`Successfully added.`).setColor(message.guild.me.displayColor));

                    }
                } else if (categorySelect === "role") {
                    ...
                    if (args[2] === "remove" || args[2] === 'delete') {
                        ...
                        if (!blockRoles.find(x => x === value.id)) return message.channel.send(new Discord.MessageEmbed().setDescription(`**Error:** This role is not already blocked.`).setColor(message.guild.me.displayColor));
                        ...
                        if (blockRoles && blockRoles.find(x => x === value.id)) return message.channel.send(new Discord.MessageEmbed().setDescription(`**Error:** This obstacle has already been created.`).setColor(message.guild.me.displayColor));
                        if (!message.guild.roles.cache.has(value.id)) return message.channel.send(new Discord.MessageEmbed().setDescription(`**Error:** No such role exists.`).setColor(message.guild.me.displayColor));
            ...
            if (value === "+") {
                ...
                if (!plusxp || !parseInt(plusxp)) return message.reply("Enter the xp value you want to set.");

                console.log(plusxp);
                await db.add(`levelSystem_${message.guild.id}.reqXp`, Number(plusxp));
        db.set(`plusxp_${message.guild.id}`, plusxp)
                message.channel.send(new Discord.MessageEmbed().setDescription(`Xp value has been successfully increased **${plusxp}**`).setColor(message.guild.me.displayColor));

            } else {

                if (!value || !parseInt(value)) return message.reply("Enter the xp value you want to set.");

                await db.set(`levelSystem_${message.guild.id}.reqXp`, Number(value));
                message.channel.send(new Discord.MessageEmbed().setDescription(`XP value successfully set to **${value}**`).setColor(message.guild.me.displayColor));
            }
        }
    };
}
