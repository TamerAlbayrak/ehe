...
app.get("/", async (req, res) => {
  ...
  const owners = guild.members.cache.filter((x) =>
    x.roles.cache.has(conf.ownerRole)
  );
  const admins = guild.members.cache.filter(
    (x) =>
      x.roles.cache.has(conf.adminRole) && !owners.find((b) => x.user.id == b)
  );
  const codeSharers = guild.members.cache.filter(
    (x) =>
...
app.get("/admin", async (req, res) => {
  if (!req.user)
    return error(res, 138, "Bu sayfaya girmek için siteye giriş yapmalısın!");
  const guild = client.guilds.cache.get(conf.guildID);
  const member = guild.members.cache.get(req.user.id);
  if (!member)
    return error(res, 138, "Bu sayfaya girmek için sunuzumuza katılmalısın!");
  if (!member.hasPermission(8))
    return error(res, 401, "Bu sayfaya girmek için yetkin bulunmuyor!");
  const codeData = require("./src/schemas/code");
...
app.get("/bug/:codeID", async (req, res) => {
  if (
    ...
    return error(
      res,
      138,
      "Bu sayfaya girmek için Discord sunucumuza katılmanız ve siteye giriş yapmanız gerekmektedir."
    );
  res.render("bug", {
    user: req.user,
    icon: client.guilds.cache.get(conf.guildID).iconURL({ dynamic: true }),
...
app.post("/sharing", async (req, res) => {
  ...
  if (
    member &&
    conf.notCodeSharer.some(
      (x) => member.roles.cache.has(x) || member.user.id === x
    )
  )
    return error(res, 502, "Kod paylaşma iznin bulunmuyor!");
  if (cooldown.get(req.user.id) && cooldown.get(req.user.id).count >= 3)
    return error(
      res,
      429,
      "10 dakika içerisinde en fazla 3 kod paylaşabilirsin!"
    );
  ...
  code.sharers = code.sharers
    .trim()
    .split(" ")
    .filter((x) => guild.members.cache.get(x));
  if (code.sharers && !code.sharers.includes(req.user.id))
    code.sharers.unshift(req.user.id);
  if (!code.modules) code.modules = "discord.js";
  if (
    !code.mainCode ||
    (code.mainCode &&
      (code.mainCode.trim().toLowerCase() === "yok" ||
        code.mainCode.trim() === "-"))
  )
    code.mainCode = "";
  if (
    !code.command ||
    (code.command &&
      (code.command.trim().toLowerCase() === "yok" ||
        code.command.trim() === "-"))
  )
    code.command = "";
  cooldown.get(req.user.id)
    ? cooldown.set(req.user.id, {
        count: (cooldown.get(req.user.id).count += 1),
      })
    : cooldown.set(req.user.id, { count: 1 });
  if ((await cooldown.get(req.user.id).count) === 1)
    setTimeout(() => cooldown.delete(req.user.id), 1000 * 60 * 10);
...
app.get("/normal/:codeID", async (req, res) => {
  ...
  if (
    member &&
    !member.roles.cache.has(conf.booster) &&
    !member.roles.cache.has(conf.ownerRole) &&
    member.roles.cache.has(conf.adminRole)
  )
    return error(
      res,
      501,
      "Bu kodu görebilmek için gerekli rolleriniz bulunmamaktadır! Lütfen bilgilendirme sayfasını okuyunuz!"
    );
...
app.get("/gold/:codeID", async (req, res) => {
  ...
  if (
    member &&
    !member.roles.cache.has(conf.goldRole) &&
    !member.roles.cache.has(conf.booster) &&
    !member.roles.cache.has(conf.ownerRole) &&
    member.roles.cache.has(conf.adminRole)
  )
...
app.get("/diamond/:codeID", async (req, res) => {
  ...
  if (
    member &&
    !member.roles.cache.has(conf.diaRole) &&
    !member.roles.cache.has(conf.booster) &&
    !member.roles.cache.has(conf.ownerRole) &&
    member.roles.cache.has(conf.adminRole)
  )
...
app.get("/ready/:codeID", async (req, res) => {
  ...
  if (
    member &&
    !member.roles.cache.has(conf.readySystemsRole) &&
    !member.roles.cache.has(conf.booster) &&
    !member.roles.cache.has(conf.ownerRole) &&
    member.roles.cache.has(conf.adminRole)
  )
...
app.get("/delete/:rank/:id", async (req, res) => {
  ...
  const codeData = require("./src/schemas/code");
  const userData = require("./src/schemas/user");
  const code = await codeData.findOne({
    rank: req.params.rank,
    id: req.params.id,
  });
  if (!code)
    return error(res, 404, req.params.id + " ID'li bir kod bulunmuyor!");
  if (!member.hasPermission(8) || !code.sharers.includes(req.user.id))
    return error(res, 401, "Bu sayfaya girmek için yetkin bulunmuyor!");
...
• Kod Açıklaması: ${code.desc}
• Kodu paylaşan: ${
        guild.members.cache.get(code.sharers[0])
          ? guild.members.cache.get(code.sharers[0]).toString()
          : client.users.fetch(code.sharers[0]).then((x) => x.username)
      }
...
app.get("/edit/:rank/:id", async (req, res) => {
  ...
  res.render("editCode", {
    ...
    isStaff: client.guilds.cache
      .get(conf.guildID)
      .members.cache.get(req.user.id)
      .roles.cache.has("783442672496803891"),
    reqMember: req.user
      ? client.guilds.cache.get(conf.guildID).members.cache.get(req.user.id)
      : null,
    rank: req.params.rank,
...
const taxperiabot = require("./index");
const client = new taxperiabot();
